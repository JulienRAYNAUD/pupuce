<?php
  ...
  if(!isset($_SESSION["user"]) || empty($_SESSION["user"])) {
    ...
    include("config.php");?>
          ...
          <html lang="fr">
            <head>
              <title>Ajout de Nourritures</title>
              <meta charset="UTF-8">
      ...
      <!--    <h1>Boutique en ligne simplifiée</h1> -->
          ...
          <hr>
            ...
            <h1>Ajouter une nourriture</h1>
            <div class="row">
              <div class="col-sm-4">
                <form id="ajouterNourriture" name="ajouterNourriture" method="post" action="ajouterNourriture.php">
                  ...
                  <button type="submit" class="btn btn-primary" name="submitNourriture" id="submitNourriture">Ajouter la nourriture</button>
      <?php
      //On vérifie si le formulaire a été validé
      if(isset($_POST['submitNourriture'])){
        ...
        $query = "INSERT INTO `produits` (`nom`, `description`, `image`, `prix`, `quantiteStock`, `TypeProduit`) VALUES ('".$_POST['nom']."', '".$_POST['description']."', '".$_POST['image']."', '".$_POST['prix']."', '".$_POST['quantiteStock']."', 'N')";
        $dbh->exec($query);
      //  echo $query;
        ?><span style="color:green";>La nourriture a bien été ajoutée !</span><?php
      }
      }
      ?>
      <br><br>
